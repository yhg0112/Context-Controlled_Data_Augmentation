...
# }
_DATA_DIR = {
    "yelp_review_subsample": "./data/text_gen/yelp"
}


class YelpReviewSubsampleConfig(datasets.BuilderConfig):
    ...
    def __init__(self, **kwargs):
        ...
        """
        super(YelpReviewSubsampleConfig, self).__init__(**kwargs)


class MyYelpReviewSubsample(datasets.GeneratorBasedBuilder):
    """Yelp Review Full Star Dataset 2015."""

    VERSION = datasets.Version("1.0.0")

    BUILDER_CONFIGS = [
        YelpReviewSubsampleConfig(
            name="yelp_review_subsample", version=VERSION, description="Yelp Review Full Star Dataset 2015"
        ),
    ]
    ...
    def _split_generators(self, dl_manager):
        ...
        data_dir = _DATA_DIR[self.config.name]

        return [
            # ls data/text_gen/yelp/yelp_review_subsample\=0.01_seed\=42_csv
            datasets.SplitGenerator(
                name=datasets.Split.TRAIN,
                gen_kwargs={
                    # "filepath": os.path.join(data_dir, "yelp_review_full_csv", "train.csv"),
                    "filepath": os.path.join(data_dir, "yelp_review_subsample=0.01_seed=42_csv", "train.csv"),
                    "split": "train",
                },
            ),
            datasets.SplitGenerator(
                ...
                gen_kwargs={"filepath": os.path.join(data_dir, "yelp_review_subsample=0.01_seed=42_csv", "test.csv"), "split": "test"},
            ),
        ]
